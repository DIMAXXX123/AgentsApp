...
app.setPath('userData', path.join(app.getPath('appData'), 'AgentsApp'));
  ...
  if (isDev) {
    ...
    mainWindow.loadFile(indexPath).catch(err => {
      ...
      const errorHtml = `
        ...
        <html>
        <head>
          <title>AgentsApp - Loading Error</title>
          <style>
  ...
  const template = [
    {
      ...
      submenu: [
        {
          label: 'About AgentsApp',
          role: 'about'
        },
        { type: 'separator' },
        {
          label: 'Hide AgentsApp',
